...
public class XyTabLayout extends HorizontalScrollView {
    ...
    /**
     * Gravity used to fill the {@link XyTabLayout} as much as possible. This option only takes effect
    ...
    /**
     * Gravity used to lay out the tabs in the center of the {@link XyTabLayout}.
     *
     * @see #setTabGravity(int)
     * @see #getTabGravity()
     */
    ...
    private boolean mColorChangeable = false;
    private int mColorFirst, mColorSecond;
    ...
    public XyTabLayout(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.XyTabLayout,
                defStyleAttr, R.style.Widget_Design_TabLayout);

        mTabStrip.setSelectedIndicatorHeight(
                a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabIndicatorHeight, 0));
        // default value was 28dp
        mTabStrip.setSelectedIndicatorWidth(
                a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabIndicatorWidth, dpToPx(28)));
        mTabStrip.setSelectedIndicatorColor(a.getColor(R.styleable.XyTabLayout_xy_tabIndicatorColor, 0));

        mTabPaddingStart = mTabPaddingTop = mTabPaddingEnd = mTabPaddingBottom = a
                .getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabPadding, 0);
        mTabPaddingStart = a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabPaddingStart,
                mTabPaddingStart);
        mTabPaddingTop = a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabPaddingTop,
                mTabPaddingTop);
        mTabPaddingEnd = a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabPaddingEnd,
                mTabPaddingEnd);
        mTabPaddingBottom = a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabPaddingBottom,
                mTabPaddingBottom);
        mSelectedBold = a.getBoolean(R.styleable.XyTabLayout_xy_tabSelectedBold,
                false);
        mTabTextAppearance = a.getResourceId(R.styleable.XyTabLayout_xy_tabTextAppearance,
                R.style.TextAppearance_Design_Tab);
        mColorChangeable = a.getBoolean(R.styleable.XyTabLayout_xy_colorChangeable, false);
        mColorFirst = a.getResourceId(R.styleable.XyTabLayout_xy_colorChangeFirst, -1);
        if (mColorFirst != 1) {
            mColorFirst = ContextCompat.getColor(context, mColorFirst);
        }
        mColorSecond = a.getResourceId(R.styleable.XyTabLayout_xy_colorChangeSecond, -1);
        if (mColorSecond != -1) {
            mColorFirst = ContextCompat.getColor(context, mColorSecond);
        }
        ...
        if (a.hasValue(R.styleable.XyTabLayout_xy_tabTextColor)) {
            // If we have an explicit text color set, use it instead
            mTabTextColors = a.getColorStateList(R.styleable.XyTabLayout_xy_tabTextColor);
        }

        if (a.hasValue(R.styleable.XyTabLayout_xy_tabSelectedTextColor)) {
            // We have an explicit selected text color set, so we need to make merge it with the
            // current colors. This is exposed so that developers can use theme attributes to set
            // this (theme attrs in ColorStateLists are Lollipop+)
            final int selected = a.getColor(R.styleable.XyTabLayout_xy_tabSelectedTextColor, 0);
            mTabTextColors = createColorStateList(mTabTextColors.getDefaultColor(), selected);
        }

        mRequestedTabMinWidth = a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabMinWidth,
                INVALID_WIDTH);
        mRequestedTabMaxWidth = a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabMaxWidth,
                INVALID_WIDTH);
        mTabBackgroundResId = a.getResourceId(R.styleable.XyTabLayout_xy_tabBackground, 0);
        mContentInsetStart = a.getDimensionPixelSize(R.styleable.XyTabLayout_xy_tabContentStart, 0);
        mMode = a.getInt(R.styleable.XyTabLayout_xy_tabMode, MODE_FIXED);
        mTabGravity = a.getInt(R.styleable.XyTabLayout_xy_tabGravity, GRAVITY_FILL);
        a.recycle();
    ...
    void setScrollPosition(int position, float positionOffset, boolean updateSelectedText,
        ...
        mCurrentOffset = positionOffset; // save offset
    ...
    /**
     * Add a {@link XyTabLayout.OnTabSelectedListener} that will be invoked when tab selection
     * changes.
     * <p>
    ...
    /**
     * Remove the given {@link XyTabLayout.OnTabSelectedListener} that was previously added via
    ...
    /**
     * Remove all previously added {@link XyTabLayout.OnTabSelectedListener}s.
     */
    ...
    /**
     * Returns the current mode used by this {@link XyTabLayout}.
     *
     * @see #setTabMode(int)
     */
    @Mode
    ...
    /**
     * The one-stop shop for setting up this {@link XyTabLayout} with a {@link ViewPager}.
     * <p>
        ...
        final void update() {
            ...
            if (mCustomView == null) {
                ...
                if (mTextView == null) {
                    ...
                    textView.setPadding(dpToPx(0), 0, dpToPx(0), 0);
                    addView(textView);
                    mTextView = textView;
    ...
    public class SlidingTabStrip extends LinearLayout {
        ...
        public void draw(Canvas canvas) {
            super.draw(canvas);
            if (mColorChangeable) {
                if (mCurrentOffset <= 0.5f && mSelectedPosition == 0) {
                    mSelectedIndicatorPaint.setColor(mColorFirst);
                    if (getTabAt(0) != null) {
                        getTabAt(0).getView().setTextColor(mColorFirst);
                    }
                    if (getTabAt(1) != null) {
                        getTabAt(1).getView().setTextColor(mTabTextColors.getDefaultColor());
                    }

                } else {
                    mSelectedIndicatorPaint.setColor(mColorSecond);
                    if (getTabAt(0) != null) {
                        getTabAt(0).getView().setTextColor(mTabTextColors.getDefaultColor());
                    }
                    if (getTabAt(1) != null) {
                        getTabAt(1).getView().setTextColor(mColorSecond);
                    }
                }
            }
     ...
     * necessary calls back to the provided {@link XyTabLayout} so that the tab position is
     * kept in sync.
     * <p>
    ...
    public static class TabLayoutOnPageChangeListener implements ViewPager.OnPageChangeListener {
        private final WeakReference<XyTabLayout> mTabLayoutRef;
        private int mPreviousScrollState;
        private int mScrollState;

        public TabLayoutOnPageChangeListener(XyTabLayout tabLayout) {
        ...
        public void onPageScrolled(final int position, final float positionOffset,
            ...
            final XyTabLayout tabLayout = mTabLayoutRef.get();
            if (tabLayout != null) {
    ...
    /**
     * A {@link XyTabLayout.OnTabSelectedListener} class which contains the necessary calls back
     * to the provided {@link ViewPager} so that the tab position is kept in sync.
     */
    public static class ViewPagerOnTabSelectedListener implements XyTabLayout.OnTabSelectedListener {
    ...
    public void setColorChangeable(boolean changeable) {
        mColorChangeable = changeable;
        invalidate();
        requestLayout();
    }

    public boolean isColorChangeable() {
        return mColorChangeable;
    }

    public void setColor(int firstColor, int secondColor) {
        mColorFirst = firstColor;
        mColorSecond = secondColor;
    }
}
